function [ runOutput ] = runNFsimOnce(pathToBNGLFile,bnglFileName,pathToNFsim,outputDirectory,runNumber,paramNames,paramValues,NumOfRuns,savedItems,steps)
...
% [status,runOutput]=system(['perl ',pathToNFsim,'/BNG2.pl "',pathToBNGLFile,bnglFileName,'"']);
% if status~=0, error(runOutput); end
% 
% 
% %Move the output file to the correct directory
% strIndex = findstr(bnglFileName,'.');
% [status,output]=system(['move "',[bnglFileName(1:strIndex-1),'.gdat'],'" "', ...
...
for i = 1:NumOfRuns
    ...
    strIndex = findstr(bnglFileName,'.');
    [status,output]=system(['move "',[bnglFileName(1:strIndex-1),'.gdat'],'" "', ...
        outputDirectory,[bnglFileName(1:strIndex-1),'_',num2str(i),'.gdat'],'"']);
    if status~=0
        error([output, 'make sure no prefix or suffix is added to the run command of the bngl file.']);
    end
end
...
%%%
%To add on additional runs to runs already completed

peace=['run'];
for i=1:length(paramNames)
    peace=[peace,'_', paramNames{i},'=',strrep(num2str(paramValues(i)),'.','_')];
end

if isfile([peace,'.mat'])
    load(peace);
    originallength= length(allValues);
    
else
    allValues = zeros(NumOfRuns,length(savedItems),steps);
    originallength=0;
end

prefix = 'PS_template_';
pathToOutput = 'templateExample/output/';

% here we then loop over the runs
for k=1:NumOfRuns
    % for each run, read in the file
    [data,~] = tblread([pathToOutput,prefix,num2str(k),'.gdat']);
    ...
    % our output array
     ...
     allValues(k+originallength,:,:) = data(:, 1:length(savedItems))'; %time

end;

save(peace,'allValues')
